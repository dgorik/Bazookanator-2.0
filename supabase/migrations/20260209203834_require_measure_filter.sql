-- Require measure filter in get_sales_by_filters
-- Returns NULL when measure is not provided to prevent returning all data
...
DECLARE
  ...
  current_quarter int;
BEGIN
  -- Return NULL if measure is not specified
...
-- Update get_top_category_sales to require measure
...
-- Update get_top_subbrand_sales to require measure
...
-- Update get_sales_by_brand to require value and target measures
