-- Top 5 div_sub winners or losers by variance (value - target)
-- Groups by div_sub, computes value & target sums, then ranks by variance.
-- p_direction = 'winners' → largest positive variance first
-- p_direction = 'losers'  → largest negative variance first
...
DECLARE
  ...
  current_quarter int;
BEGIN
  -- Return empty if measures are not specified
