...
BEGIN
  ...
  SELECT
    pd.brand,
    COALESCE(SUM(CASE WHEN pd.measure = p_value_measure AND pd.year = p_value_measure_year THEN pd.sales ELSE 0 END), 0) as value_measure,
    COALESCE(SUM(CASE WHEN pd.measure = p_target_measure AND pd.year = p_target_measure_year THEN pd.sales ELSE 0 END), 0) as target_measure
